...
#example: Rscript ./10X_scRNA_QC_filtering_Seurat_v3_reanalyze_titan_2020.R ./Lipo-Fibroblast_results/outs/filtered_feature_bc_matrix/ Lipo-Fibroblast -2
...
barcode<-colnames(temp)
barcode<-data.frame(Barcode=barcode)
#barcode[,1]<-paste(barcode[,1],"-1",sep="") #no need to add "-1" for seurat 3.1.5
write.csv(barcode,paste(name,"barcode_filtered_gene300_mt15.csv",sep="_"),row.names = F,quote = F)
}
...
############################## MAD method filtering ##############################
  ...
  #barcode_MAD[,1]<-paste(barcode_MAD[,1],"-1",sep="")  #no need to add "-1" for seurat 3.1.5
