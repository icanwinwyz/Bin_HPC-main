#example: Rscript ./scRNA_CITE_QC_reanalysis.R ./Lipo-Fibroblast_results/outs/filtered_feature_bc_matrix/ Lipo-Fibroblast CITEseq
#setwd("~/Documents/project_tracking/Goodridge_Helen/HG-9299--03--25--2020/filtered_feature_bc_matrix/")
library(Seurat)
library(dplyr)
library(Matrix)
library(methods)
library(gplots)
library(scater)
packageVersion("Seurat") #‘3.1.1’


args=commandArgs(TRUE)
path<-args[1]
#path <- "~/Documents/project_tracking/Goodridge_Helen/HG-9299--03--25--2020/filtered_feature_bc_matrix/"
name<-args[2]
#name <- "Young-Old-Monocytes"
#type<-args[3]
seq <-args[3] #CITEseq or others
...
if (seq == "CITEseq") {
  ...
  #feature.names$V4 = paste(feature.names$V2, feature.names$V1, sep = "_")
  #rownames(mat) = feature.names$V4

  pattern <- "Hashtag"
  antibody <- c("Flt3\\(CD135\\)_TotalA",
  "CD115_TotalA",
  "Ly6C_TotalA",
  "CD11b_TotalA",
  "CD11c_TotalA",
  "I-A\\(I-E\\)\\(MHC-II\\)_TotalA")
  #antibody <- "ADT"
  mat_tag <- mat[grep(pattern, rownames(mat), value = TRUE), ] 
  mat_AD <-  mat[grep(paste(antibody, collapse="|"), rownames(mat), value = TRUE), ]
  #mat_RNA <- mat[1:27998, ]
  mat_RNA <- mat[grep(paste(c(pattern,antibody),collapse="|"), rownames(mat), value = TRUE, invert=TRUE), ]
  
  ### creat seurat object
  test <- CreateSeuratObject(counts = mat_RNA, min.cells = 0, project = name) 
  test[["HTO"]] <- CreateAssayObject(counts = mat_tag)
  test[["ADT"]] <- CreateAssayObject(counts = mat_AD)
} else {
...
if (seq == "CITEseq") {
  ...
  #normalize HTO and ADT
  tag <- NormalizeData(tag, assay = "HTO", normalization.method = "CLR")
  tag <- NormalizeData(tag, assay = "ADT", normalization.method = "CLR")
  expr_raw <- GetAssayData(object = tag, slot = "counts")
  expr_norm <- GetAssayData(object = tag, slot = "data")
  HTO_raw <- GetAssayData(object = tag, assay = "HTO", slot = "counts")
  HTO_norm <- GetAssayData(object = tag, assay = "HTO", slot = "data")
  ADT_raw <- GetAssayData(object = tag, assay = "ADT", slot = "counts")
  ADT_norm <- GetAssayData(object = tag, assay = "ADT", slot = "data")
  tag_raw <- rbind(expr_raw, HTO_raw, ADT_raw)
  tag_norm <- rbind(expr_norm, HTO_norm, ADT_norm)
  } else {
    tag<-NormalizeData(object = test_1)
...
col.num_MAD <- which(colnames(test) %in% barcode_MAD$Barcode)
...
if (seq == "CITEseq") {
  ...
  #normalize HTO and ADT
  NewDF_MAD <- NormalizeData(NewDF_MAD, assay = "HTO", normalization.method = "CLR")
  NewDF_MAD <- NormalizeData(NewDF_MAD, assay = "ADT", normalization.method = "CLR")
  expr_raw <- GetAssayData(object = NewDF_MAD, slot = "counts")
  expr_norm <- GetAssayData(object = NewDF_MAD, slot = "data")
  HTO_raw <- GetAssayData(object = NewDF_MAD, assay = "HTO", slot = "counts")
  HTO_norm <- GetAssayData(object = NewDF_MAD, assay = "HTO", slot = "data")
  ADT_raw <- GetAssayData(object = NewDF_MAD, assay = "ADT", slot = "counts")
  ADT_norm <- GetAssayData(object = NewDF_MAD, assay = "ADT", slot = "data")
  NewDF_MAD_raw <- rbind(expr_raw, HTO_raw, ADT_raw)
  NewDF_MAD_norm <- rbind(expr_norm, HTO_norm, ADT_norm)
