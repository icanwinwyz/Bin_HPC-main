...
{
	execute("qsub -q all.q -l h=$sort[$index] -N genome_load_$sort[$index] -sync y -cwd $cmd_path/loading-setx.sh $organism $sort[$index]");
...
#	my $mapping_command = "qsub -q all.q -l h=$sort[$nodeindex] -pe smp 3 -N mapping_$sort[$nodeindex] -cwd $cmd_path/mapping_star_notarray_setx.sh $organism $seq $data[$index]";
	my $mapping_command = "qsub -q all.q -l h=$sort[$nodeindex] -pe smp 10 -N mapping -cwd $cmd_path/mapping_star_notarray_setx.sh $organism $seq $data[$index]";
...
for ($index=0;$index<$nodes_number;$index++)
{
	...
	execute("qsub -q all.q -l h=$sort[$index] -N remove_ref -hold_jid mapping -sync y -cwd $cmd_path/remove_ref.sh $organism $sort[$index]");
...
for ($index=0;$index<$number;$index++)
{
	...
	execute("qsub -q all.q -N rsem_cal -hold_jid remove_ref -pe smp 5 -cwd $cmd_path/rsem_cal.sh $organism $seq $data[$index]");
...
####RseQC####
	for ($index=0;$index<$number;$index++){
		$nodeindex=$index % $nodes_number;
		print $data[$index],"\n";
        	my $rseqc_comman1 = "qsub -q all.q -hold_jid fastqc -pe smp 5 -N rseqc -cwd $cmd_path/rseqc.sh $organism $seq $data[$index]";
