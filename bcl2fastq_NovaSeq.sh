...
bcl2fastq -R $SEQ_PATH/$2 -o $FOLDER_PATH/$FN -r 5 -p 5 -w 5 --barcode-mismatches 0 --ignore-missing-controls --ignore-missing-bcls --ignore-missing-positions --ignore-missing-filter --no-lane-splitting  &> "$FOLDER_PATH/$FN/demultiplex_log.txt"

mv Undetermined*.fastq.gz Undetermined_Fastq

sed '1,/Sample_ID/d' SampleSheet.csv > samplesheet_tmp.csv

bash /home/genomics/genomics/apps/adapter_trim/integrate.sh $FOLDER_PATH/$FN
...
if [ -f $CHECK_FILE ]; then
		...
		/usr/bin/perl /home/genomics/bin/bcl_summary_mail.pl $FOLDER_PATH/$FN/Stats/DemultiplexingStats.xml $FOLDER_PATH/$FN/ $2 NovaSeq $FOLDER_PATH/$FN/Stats/Stats.json $FOLDER_PATH/$FN/SampleSheet.csv
		...
		/usr/bin/perl /home/genomics/bin/bcl_summary_mail.pl $FOLDER_PATH/$FN/Stats/DemultiplexingStats.xml $FOLDER_PATH/$FN/Data_processing $2 NextSeq $FOLDER_PATH/$FN/Stats/Stats.json $FOLDER_PATH/$FN/SampleSheet.csv

		elif [ "$1" = "MiSeq" ]; then

                /usr/bin/perl /home/genomics/bin/bcl_summary_mail.pl $FOLDER_PATH/$FN/Stats/DemultiplexingStats.xml $FOLDER_PATH/$FN/ $2 MiSeq $FOLDER_PATH/$FN/Stats/Stats.json $FOLDER_PATH/$FN/SampleSheet.csv
