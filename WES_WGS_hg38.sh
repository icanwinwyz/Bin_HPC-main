#!/bin/bash
###$1 is the sample name; $2 is the Read1 and $3 is the Read2
...
#echo "Subject: Faidx is done for REF" | sendmail -v di.wu@cshs.org
...
#echo "Subject: Create SequenceDictionary is done for REF" | sendmail -v di.wu@cshs.org
...
echo "Subject: Mapping is done for $1" | sendmail -v di.wu@cshs.org
...
echo "Subject: Sorting is done for $1" | sendmail -v di.wu@cshs.org
...
echo "Subject: Collect Alignment & Insert Size Metrics is done for $1" | sendmail -v di.wu@cshs.org
...
echo "Subject: Depth calculation is done for $1" | sendmail -v di.wu@cshs.org
...
echo "Subject: Mark Duplicates is done for $1" | sendmail -v di.wu@cshs.org
#echo "Checkpoint 1 for $1"|sendmail -v di.wu@cshs.org
...
echo "Subject: BuildBamIndex is done for $1" | sendmail -v di.wu@cshs.org
...
echo "Subject: RealignerTargetCreator is done for $1" | sendmail -v di.wu@cshs.org
...
echo "Subject: Realign Indels is done for $1" | sendmail -v di.wu@cshs.org
...
echo "Subject: Call Variants is done for $1" | sendmail -v di.wu@cshs.org
...
echo "Subject: Extract SNPs & Indels is done for $1" | sendmail -v di.wu@cshs.org
...
echo "Subject: 	Filter SNPs is done for $1" | sendmail -v di.wu@cshs.org
...
echo "Subject: Filter Indels is done for $1" | sendmail -v di.wu@cshs.org
...
echo "Subject: 	Base Quality Score Recalibration (BQSR) is done for $1" | sendmail -v di.wu@cshs.org
...
echo "Subject: Post Base Quality Score Recalibration  is done for $1" | sendmail -v di.wu@cshs.org
...
echo "Subject: Analyze Covariates is done for $1" | sendmail -v di.wu@cshs.org
...
echo "Subject: Apply BQSR is done for $1" | sendmail -v di.wu@cshs.org
...
echo "Subject: Extract SNPs is done for $1" | sendmail -v di.wu@cshs.org
...
echo "Subject: Extract Indels is done for $1" | sendmail -v di.wu@cshs.org
...
echo "Subject: 	Filter SNP is done for $1" | sendmail -v di.wu@cshs.org

echo "##############Filter Indels is started"
/common/genomics-core/anaconda2/bin/java  -jar $PICARD/GenomeAnalysisTK.jar -T VariantFiltration -R $REF/GCA_000001405.15_GRCh38_no_alt_analysis_set.fna -V $1_raw_indels_recal.vcf --filterExpression 'QD < 2.0 || FS > 200.0 || ReadPosRankSum < -20.0 || SOR > 10.0' --filterName "basic_indel_filter" -o $1_filtered_indels_final.vcf
echo "Subject: 	Filter Indels is done for $1" | sendmail -v di.wu@cshs.org
...
echo "Subject: 	SNP anno is done for $1" | sendmail -v di.wu@cshs.org
...
echo "Subject: GATK eval is done for $1" | sendmail -v di.wu@cshs.org
